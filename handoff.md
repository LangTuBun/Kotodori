...
- `pos: 'unknown'` on 152 MD-sourced vocab entries
- **`chapter: 0`** on 68 vocab entries; 56 of those also have corrupted `kanji`/`kana` fields from a pre-repo MD ingestion bug (e.g. `n5_0974`) — needs a dedicated cleanup pass
- **`n5_1051`** is a corrupted duplicate of `n5_0652` — should be deleted
- **~17 `vocabulary.json` entries** have a doubled/concatenated `kana` field (e.g. 名前 → `おなまえなまえ`) — pre-existing PDF-extraction artifact
